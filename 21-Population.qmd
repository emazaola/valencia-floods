...
Se descargan las capas de GHS-POP de los años que interesan a la resolución que interesa.
...
path <- "../../data/geodata/GHS_POP/" # la ruta donde se encuentran los archivos  GHS_POP
```
...
plot(AOI_stack, 
     ...
     axes = TRUE,
     nlyr(AOI_stack)) # plot la última capa del stack
```
...
plot(AOI_stack, 
     range=c(0.1,180), # para que tengan todos los plt la misma escala
     col=paleta_POP(255),
...
writeRaster(AOI_stack,
         paste0("../../process/geoprocess/GHS_POP/",
                aoi_name,
                "_GHS_POP.tif"),
...
for (i in 1:nlyr(AOI_stack)) {
  writeRaster(AOI_stack[[i]], 
              filename = paste0("../../process/geoprocess/GHS_POP/",
                                names(AOI_stack)[i], "_",
                                aoi_name,
                                ".tif"),
              overwrite=TRUE)  # para remplazar)
...
```{r diferencia_2030-POP}
diff_2030 <- AOI_stack$GHS_POP_E2030 - AOI_stack$GHS_POP_E2020

plot(diff_2030, 
     range=c(-5,40), # para que tengan todos los plt la misma escala
     col=paleta_POP_diff(255),
     1:nlyr(diff_2030)) # plot todas las capas del stack 
```
